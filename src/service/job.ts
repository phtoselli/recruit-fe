...
export const getJobs = async () => {
  ...
  try {
    ...
    return response.data; // Isso deve retornar um array
  } catch (error: any) {
    message.destroy();
    message.error(error.message || "Erro ao buscar por vagas.", 4);
    return undefined; // Retorne undefined ou um valor padrão se houver erro
  }
};
...
export const getUsersByJob = async (jobId: string) => {
  message.loading('Buscando usuários por vaga...', 0);
  try {
    const response = await axios.get(`${API_URL}/job/${jobId}/users`);
    message.destroy();
    message.success('Usuários encontrados com sucesso!', 4);
    return response.data; // Certifique-se de que isso está aqui
  } catch (error: any) {
    message.destroy();
